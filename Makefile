SKETCHES = DebugSerial HelloSerial
...
DEPLOY_CMD ?= cp -r
DEPLOY_DIR ?= ~/Arduino/libraries/MicroSerial
...
.PHONY: build
build: build/rpipico build/esp32c3 build/uno build/nano build/leonardo build/promicro build/attiny85 build/attiny13
...
.PHONY: install/gcc
install/gcc:
	sudo apt update && sudo apt-get install -y gcc-11 make
